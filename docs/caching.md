...
Rail has three independent cache boundaries. They share exact-byte identities and local CAS storage where
...
All three follow one rule: a result is reused only after every input that produced it is re-verified. Anything Rail
cannot verify runs cold through the normal tool, with a stable reason you can inspect. A wrong reuse is treated as a
correctness bug, never as an acceptable speed tradeoff.
...
another compiler wrapper — if sccache or a custom wrapper is present, Rail preserves it and steps aside rather than
double-caching.

Initialize Rail, then run the same action normally:
...
Use `--no-cache` for an intentional Rail baseline:
...
Cargo still creates and validates its own fingerprints around every wrapper invocation. Rail never restores a
...
Other hosts remain `platform_limited` until Rail can enforce an equivalent filesystem and network boundary.
...
- `cargo rail clean --cache` removes only a Rail-owned root reached through its validated reference and owner
  marker.
...
| check | Rail disabled | 7.397 s | 8.183 s |
| check | Rail cold | 9.602 s | 10.115 s |
| check | Rail warm cross-root | 4.979 s | 5.615 s |
| check | sccache 0.16.0 | 5.278 s | 5.999 s |
| release build | native Cargo | 10.369 s | 10.808 s |
| release build | Rail disabled | 10.947 s | 11.262 s |
| release build | Rail cold | 14.336 s | 15.173 s |
| release build | Rail warm cross-root | 7.630 s | 8.347 s |
| release build | sccache 0.16.0 | 8.173 s | 8.902 s |

Warm Rail saved 29.2% at check p50 and 26.4% at release-build p50 versus native Cargo. It saved 48.1% and 46.8%
respectively versus cold publication; two warm reuses repaid the measured cold overhead for both workloads. Against
sccache, warm Rail was 5.7% faster for checks and 6.6% faster for release builds at p50.

Dedicated AWS runs using the same 110-sample contract also qualified on Linux x86-64 and Linux ARM64:

| Host | Workload | Native p50/p95 | Rail warm p50/p95 | sccache p50/p95 |
|---|---|---:|---:|---:|
...
Warm Rail beat native Cargo on every qualified host and workload. Against sccache, it won both macOS workloads
...
Existing sccache installations remain preserved and cause Rail native reuse to bypass.
...
“Arbitrary toolchain support” cannot mean accepting an unknown compiler. Rail qualifies an exact toolchain
