...
pub(crate) fn publish(invocation: &CompilerFactInvocation, object: CompilerFactObject) -> Result<(), String> {
  let object_bytes = serde_json::to_vec(&object).map_err(|error| format!("serialize compiler fact object: {error}"))?;
  ...
  match fs::rename(&temporary_path, &final_path) {
    ...
    Err(error) if final_path.is_file() => {
      let existing = fs::read(&final_path).map_err(|read| format!("read existing compiler fact sidecar: {read}"))?;
      if existing != bytes {
        return Err("content-addressed compiler fact sidecar has conflicting bytes".to_string());
      }
      fs::remove_file(&temporary_path).map_err(|remove| format!("remove duplicate compiler fact sidecar: {remove}"))?;
      let _ = error;
    }
  ...
  let payload =
    serde_json::to_string(&announcement).map_err(|error| format!("serialize compiler fact announcement: {error}"))?;
  let diagnostic = json!({
