...
use crate::fact_protocol::{
  COMPILER_FACT_PROTOCOL_VERSION, CompilerFactCompletion, CompilerFactCoverage, CompilerFactEdge, CompilerFactEdgeKind,
  CompilerFactEntryPoint, CompilerFactEntryPointKind, CompilerFactInvocation, CompilerFactItemKind,
  CompilerFactMacroProvenance, CompilerFactNamespace, CompilerFactObject, CompilerFactPhysicalIdentity,
  CompilerFactRetention, CompilerFactRetentionReason, CompilerFactSource, CompilerFactSourcePath, CompilerFactSpan,
  CompilerFactStringId, CompilerFactVisibility, CompilerItemFact, CompilerItemId,
};
...
pub(crate) fn collect(tcx: TyCtxt<'_>, invocation: &CompilerFactInvocation) -> Result<CompilerFactObject, String> {
  ...
  let source_root =
    fs::canonicalize(&invocation.source_root).map_err(|error| format!("resolve authorized source root: {error}"))?;
  let generated_roots = invocation
    .generated_roots
    .iter()
    .map(|root| fs::canonicalize(root).map_err(|error| format!("resolve authorized generated root '{root}': {error}")))
...
impl<'tcx> Collector<'tcx> {
  ...
  fn collect_retentions(
    ...
    for def_id in definitions {
      ...
      if self
        .tcx
        .lint_level_at_node(DEAD_CODE, self.tcx.local_def_id_to_hir_id(*def_id))
        .level
