...
fn assert_task_local_expansion(fragment: &CompilerFactFragment) {
    // This is the release-readiness reproducer. The pinned compiler retains
    // exact generated bytes, so those definitions stay under the existing
    // strict span authority instead of widening the wire contract.
