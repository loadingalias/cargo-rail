...
#[test]
fn test_plan_from_nested_workspace_strips_workspace_prefix() {
    let result: Result<()> = (|| {
...
#[test]
fn test_nested_workspace_source_capture_stays_repository_relative() {
    let result: Result<()> = (|| {
        let ws = NestedWorkspace::new("rust")?;
        ws.add_crate("lib-a", "0.1.0")?;
        ws.commit("add lib-a")?;
...
#[test]
fn test_nested_workspace_excludes_only_its_resolved_generated_roots() {
    let result: Result<()> = (|| {
        let ws = NestedWorkspace::new("rust")?;
        ws.add_crate("lib-a", "0.1.0")?;
...
#[test]
fn test_nested_workspace_rejects_a_cargo_target_root_that_contains_source() {
    let result: Result<()> = (|| {
        let ws = NestedWorkspace::new("rust")?;
        ws.add_crate("lib-a", "0.1.0")?;
...
#[test]
fn test_workspace_outside_git_worktree_is_rejected() {
    let result: Result<()> = (|| {
