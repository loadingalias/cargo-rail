...
#[test]
fn ownership_uses_the_deepest_exact_package_root_without_filesystem_access() {
    let result: Result<()> = (|| {
...
#[test]
fn ownership_maps_root_package_files() {
    let result: Result<()> = (|| {
...
#[test]
fn ownership_accepts_exact_absolute_roots_for_external_workspace_members() {
    let result: Result<()> = (|| {
