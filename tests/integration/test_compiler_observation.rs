...
fn cache_disabled_wrapper_preserves_the_process_contract() -> Result<()> {
  ...
  let working_directory_output = root.path().join("compiler-working-directory");
...
pwd -P > "$WORKING_DIRECTORY_OUTPUT"
...
"#,
  ...
  let output = Command::new(env!("CARGO_BIN_EXE_cargo-rail"))
    ...
    .env("WORKING_DIRECTORY_OUTPUT", &working_directory_output)
  ...
  assert_eq!(
    fs::canonicalize(String::from_utf8(fs::read(&working_directory_output)?)?.trim())?,
