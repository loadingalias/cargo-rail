...
fn stable_wrapper_authorizes_the_matched_driver_per_compilation_unit() -> Result<()> {
...
fn stable_wrapper_typed_fixture(doctest: bool) -> Result<Vec<serde_json::Value>> {
  ...
  if doctest {
    ...
    let announced_targets = announcements
      ...
      .collect::<std::collections::BTreeSet<_>>();
    assert!(announced_targets.contains("fact_macro"));
    assert!(announced_targets.contains("fact_probe"));
  }
  ...
  if doctest {
    for fragment in &fragments {
      assert_eq!(fragment["object"]["unit"]["domain"], "doctest");
    }
  } else {
...
#[test]
fn overlapping_observation_markers_dispatch_by_cargo_wrapper_shape() -> Result<()> {
