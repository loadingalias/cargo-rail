...
#[test]
fn resolution_views_are_lazy_exact_and_single_flight_cached() {
    let result: Result<()> = (|| {
...
#[test]
fn native_default_resolution_remains_available_without_snapshot_capture() {
    let result: Result<()> = (|| {
...
#[test]
fn resolution_view_rejects_inexact_package_and_credential_url_identity() {
    let result: Result<()> = (|| {
