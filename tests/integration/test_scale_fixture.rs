...
#[test]
fn scale_fixtures_are_deterministic_and_cargo_valid() {
    let result: Result<()> = (|| {
...
#[test]
fn scale_fixture_generator_refuses_nonempty_destinations() {
    let result: Result<()> = (|| {
        let root = TempDir::new()?;
...
#[test]
fn semantic_impact_narrows_100_and_1000_member_mutations_without_missing_the_witness() {
    let result: Result<()> = (|| {
        let root = TempDir::new()?;
