  ...
  let local_fallbacks = events
    ...
    .filter(|event| {
      event["status"] == "miss"
        && event["reason"]
          .as_str()
          .is_some_and(|reason| reason.starts_with("distributed_transport_unavailable;stored_verified_result"))
    })
    .count();
  anyhow::ensure!(
...
fn execution_request_with_emission(
  ...
  let operation = RustLibraryOperation {
    ...
    rlib_name: (emission == "metadata_and_link").then(|| "libdistributed_fixture-0123456789abcdef.rlib".to_string()),
