...
if [[ ! "$run_id" =~ ^task10-[A-Za-z0-9][A-Za-z0-9._-]*$ || "$run_id" == *..* ]]; then
  echo "usage: $0 <task10-run-id>" >&2
  exit 2
fi

results_root="$repo_root/benchmark_results"
run="$results_root/distributed-execution/$run_id"
...
[[ -d "$run" ]] || {
  echo "distributed-execution run does not exist: $run" >&2
  exit 2
}
...
if [[ -f "$archive" && -f "$manifest" ]]; then
  ...
  if [[ "$expected_name" == "$run_id.tar" && "$(sha256_file "$archive")" == "$expected_digest" ]]; then
    echo "distributed-execution archive: keeping complete $run_id" >&2
    exit 0
  fi
fi
...
tar -cf "$archive_staging" -C "$results_root" "distributed-execution/$run_id"
...
echo "distributed-execution archive: finalized $run_id ($digest)" >&2
