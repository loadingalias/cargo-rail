...
run="$results_root/compiler-facts/$run_id"
...
[[ -d "$run" ]] || {
  echo "compiler-fact run does not exist: $run" >&2
  exit 2
}
...
if [[ -f "$archive" && -f "$manifest" ]]; then
  ...
  if [[ "$expected_name" == "$run_id.tar" && "$(sha256_file "$archive")" == "$expected_digest" ]]; then
    echo "compiler-fact archive: keeping complete $run_id" >&2
    exit 0
  fi
fi
...
tar -cf "$archive_staging" -C "$results_root" "compiler-facts/$run_id"
...
echo "compiler-fact archive: finalized $run_id ($digest)" >&2
