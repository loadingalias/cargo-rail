...
selection = {'source_identity': source_identity, 'rustc_verbose': verbose, 'sysroot': str(sysroot), 'compiler_library_digest': library_digest}
...
if record_path.is_file() and not record_path.is_symlink():
    ...
    if record.get('selection') == selection and set(record.get('artifacts', {})) == {driver_name, 'cargo-rail-fact-driver-source-v1.json', env_path.name} and all(
...
with tempfile.TemporaryDirectory(prefix='.cargo-rail-driver-prepare-', dir=destination) as temporary:
    ...
    if source_bytes != {path.relative_to(root).as_posix(): path.read_bytes() for path in sorted(sources)} or library_digest != digest(library.read_bytes()) or verbose != run(['rustc', '-vV']):
    ...
    names = (driver_name, source_name, env_path.name)
