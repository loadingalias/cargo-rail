...
pub fn find_last_release_tags(
    repo_path: &Path,
    crate_names: &[String],
) -> RailResult<HashMap<String, ReleaseTag>> {
...
pub fn detect_changed_crates(
    ...
    for crate_name in crate_names {
        let has_changes = if let Some(tag) = last_tags.get(crate_name) {
            ...
            let crate_path = crate_paths.get(crate_name).ok_or_else(|| {
                anyhow::anyhow!("No path found for crate '{}'", crate_name)
            })?;

            commits_since_tag.iter().any(|commit_sha| {
                git.get_changed_files(commit_sha)
                    .ok()
                    .map(|files| {
                        files
                            .iter()
                            .any(|(path, _)| path.starts_with(crate_path.as_str()))
                    })
                    .unwrap_or(false)
            })
        } else {
