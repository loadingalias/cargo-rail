        ...
        let results: Vec<RailResult<ParsedManifest>> = members
            ...
            .map(|pkg| {
                ...
                let required_feature_selections = pkg
                    ...
                    .collect();
                let source_features =
                    crate::cargo::feature_scanner::scan_snapshot_source_features(snapshot, package_root)?;
                Self::parse_manifest(
            ...
            .map(|(field, workspace_value)| {
                ...
                for member in &self.members {
                    match member.package_fields.get(field) {
                        Some(PackageFieldDeclaration::Inherited) => {
                            analysis.inherited.push(member.package_name.clone())
                        }
                        ...
                        Some(PackageFieldDeclaration::Explicit(_)) => {
                            analysis.local_overrides.push(member.package_name.clone())
                        }
