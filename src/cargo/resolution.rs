...
struct WrappedRustcQuery<'a> {
    cargo_program: &'a OsStr,
    rustc_program: &'a OsStr,
    rustc_wrapper_program: Option<&'a OsStr>,
    rustc_workspace_wrapper_program: Option<&'a OsStr>,
    workspace_root: &'a Path,
    cargo_config: &'a CargoConfigSnapshot,
}

impl WrappedRustcQuery<'_> {
    fn run(&self, argument: &str, description: &str) -> RailResult<String> {
        let mut command = configured_rustc_command(
            self.rustc_program,
            self.rustc_wrapper_program,
            self.rustc_workspace_wrapper_program,
        );
        apply_cargo_environment(&mut command, self.cargo_config)?;
        command.env("CARGO", self.cargo_program);
...
impl CargoConfigSnapshot {
    ...
    fn config_string_with_source(&self, path: &[&str]) -> RailResult<Option<(&str, &CargoConfigSource)>> {
        ...
        let value = value.as_str().ok_or_else(|| {
            RailError::message(format!("Cargo configuration key '{}' must be a string", path.join(".")))
        })?;
        let source = self
            .provenance
            .iter()
        ...
        for source in &self.provenance {
            ...
            let values = if let Some(value) = value.as_str() {
                ...
                value
                    .as_array()
                    .ok_or_else(|| {
                        RailError::message("Cargo configuration key 'build.target' must be a string or string array")
                    })?
                    .iter()
                    .map(|value| {
                        value
                            ...
                            .ok_or_else(|| {
                                RailError::message(
                                    "Cargo configuration key 'build.target' contains an empty or non-string target",
                                )
                            })
                    })
    ...
    pub(crate) fn effective_environment_value(&self, name: &str) -> RailResult<Option<OsString>> {
        ...
        if relative {
            let (_, source) = self.config_string_with_source(&value_path)?.ok_or_else(|| {
                RailError::message(format!("Cargo configuration key 'env.{name}' has no path provenance"))
            })?;
...
impl ResolutionViews {
    ...
    fn new_inner(
        ...
        let (cargo_config, toolchain) = inputs.map_or_else(
            || (OnceLock::new(), OnceLock::new()),
            |inputs| {
                (
                    OnceLock::from(Ok(inputs.cargo_config)),
                    OnceLock::from(Ok(inputs.toolchain)),
                )
            },
        );
        Self {
...
fn command_options(request: &ResolutionRequest, metadata: &Metadata) -> RailResult<ResolutionCommandOptions> {
    ...
    let package_by_id: FxHashMap<&PackageId, &Package> =
        packages.iter().map(|package| (&package.id, *package)).collect();
...
impl ToolchainIdentity {
    fn capture(cargo_current_dir: &Path, cargo_config: &CargoConfigSnapshot) -> RailResult<Self> {
        ...
        let wrapped_rustc = WrappedRustcQuery {
            cargo_program: &cargo_program,
            rustc_program: &rustc_program,
            rustc_wrapper_program: rustc_wrapper_program.as_deref(),
            rustc_workspace_wrapper_program: rustc_workspace_wrapper_program.as_deref(),
...
fn apply_cargo_environment(command: &mut Command, cargo_config: &CargoConfigSnapshot) -> RailResult<()> {
    ...
    for (name, setting) in environment {
        ...
        if relative {
            let (_, source) = cargo_config.config_string_with_source(&value_path)?.ok_or_else(|| {
                RailError::message(format!("Cargo configuration key 'env.{name}' has no path provenance"))
            })?;
    ...
    values
        ...
        .map(|value| {
            value.as_str().map(str::to_string).ok_or_else(|| {
                RailError::message(format!("Cargo configuration key '{key}' contains a non-string value"))
            })
        })
        .collect()
}
...
fn wrapped_rustc_identity(query: &WrappedRustcQuery<'_>) -> RailResult<String> {
    query.run("-vV", "wrapped 'rustc -vV'")
}
...
impl CargoConfigSnapshot {
    ...
    pub(crate) fn capture(cargo_current_dir: &Path) -> RailResult<Self> {
        ...
        let (credential_capabilities, credential_provenance) =
            capture_credential_capabilities(&cargo_home, &mut framed)?;
...
mod tests {
    ...
    fn credential_alias_commands_never_enter_sanitized_config() {
        ...
        let sanitized = sanitize_config_value(&value, "credential-alias.private")
            .expect("credential alias should become a capability");
    ...
    fn legacy_cargo_credentials_file_has_cargos_documented_precedence() {
        ...
        let mut framed = Vec::new();
        let (capabilities, provenance) = capture_credential_capabilities(cargo_home.path(), &mut framed)
            .expect("credential capability should capture");
        assert_eq!(
            provenance.as_deref(),
            Some(
    ...
    fn cargo_config_merge_rejects_incompatible_types() {
        ...
        let error = merge_config_value(&mut effective, &invalid, "")
            .expect_err("incompatible Cargo setting types must fail closed");
    ...
    fn cargo_flag_environment_and_cli_precedence_match_cargo_rustc() {
        ...
        let modeled = effective_flags(&config, rustc_host(), None, CompilerFlags::Rust)
            .expect("RUSTFLAGS precedence should resolve");
...
"#,
        ...
        let targets =
            capture_target_identities(workspace.path(), &[], &inputs).expect("target identity should resolve");
        ...
        let wrappers =
            fs::read_to_string(workspace.path().join("wrapper.log")).expect("wrapper log should be readable");
...
"#
        ...
        let runner_argv =
            fs::read_to_string(workspace.path().join("runner.log")).expect("runner log should be readable");
...
"#
    ...
    fn unstable_host_configuration_fails_closed() {
        ...
        let inputs = target_test_inputs(workspace.path());
        let error = capture_target_identities(workspace.path(), &[], &inputs)
            .expect_err("unstable host semantics must fail closed");
