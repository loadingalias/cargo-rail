    ...
    for declaration in declarations {
      ...
      for usage in declaration.usages {
        ...
        let optional_proof = if usage.optional {
          ...
          if member_evidence.dependency_state_for_features(&dependency, &applicable_targets, &required_features)
            != DependencyEvidenceState::Unused
          {
            continue;
          }
        ...
        for usage in usages {
          let reason = if usage.referenced_in_features {
            ...
            match usage.kind {
              DepKind::Normal => None,
              DepKind::Dev => None,
              DepKind::Build if has_links => Some("the dependency declares a native links contract with side effects"),
              DepKind::Build => None,
            }
          };
