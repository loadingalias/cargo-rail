...
struct BorrowedFeatureUsage<'a> {
    member: &'a crate::cargo::manifest_analyzer::ParsedManifest,
    dependency: &'a crate::cargo::manifest_analyzer::DepKey,
    usage: &'a crate::cargo::manifest_analyzer::DepUsage,
    feature_sources: &'a FxHashMap<String, Vec<FeatureProvider>>,
    resolved_features: &'a BTreeSet<String>,
    default_features: &'a BTreeSet<String>,
}
...
impl UnifyAnalyzer {
    ...
    fn normalize_dep_path(&self, member_manifest_path: &Path, dep_path: &str) -> PathBuf {
        ...
        let canonical_dep = absolute_dep_path
            .canonicalize()
            .unwrap_or_else(|_| absolute_dep_path.clone());
    ...
    pub fn analyze(&self) -> RailResult<UnificationPlan> {
        ...
        for candidate in CandidateIterator::new(&self.manifests, &self.config) {
            ...
            let version = if versions.is_empty() {
                ...
                match unique_versions.iter().max() {
                    Some(max) if unique_versions.len() > 1 => {
                        ...
                        let mut versions_found: Vec<Arc<str>> =
                            unique_versions.iter().map(|v| Arc::from(v.to_string())).collect();
                        versions_found.sort();
            ...
            let mut aliases =
                std::collections::BTreeMap::<Arc<str>, Vec<&crate::cargo::manifest_analyzer::DepUsage>>::new();
            for usage in &usage_sites {
                aliases
            ...
            for usage in usage_sites {
                ...
                if !*default_features
                    ...
                    && !local_features.iter().any(|feature| &**feature == "default")
                {
        ...
        unused_deps.retain(|unused| {
            !undeclared_features.iter().any(|feature| {
                let same_declaration = feature.member == unused.member && feature.dep_name == unused.dep_name;
                let same_dependency_kind = feature.dep_kind == unused.kind;
                same_declaration && same_dependency_kind
            })
        });
                ...
                usages.iter().any(|usage| {
                    usage.optional
                        && feature.declared_edges.iter().any(|edge| {
                            crate::cargo::feature_scanner::feature_edge_references_dependency(
                                edge,
                                &usage.cargo_toml_key,
                            )
                        })
        ...
        for member in &self.manifests.members {
            for (dep_key, usages) in &member.dependencies {
                ...
                for usage in usages {
                    ...
                    let feature_usage = BorrowedFeatureUsage {
                        member,
                        dependency: dep_key,
                        usage,
                        feature_sources: feat_sources,
                        resolved_features: &resolved_features,
                        default_features: &default_features,
                    };
                    if let Some(borrowed) =
                        self.find_borrowed_features_for_usage(feature_usage, &mut skipped_platform_features)
                    {
