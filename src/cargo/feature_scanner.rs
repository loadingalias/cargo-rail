...
fn extract_cfg_expressions(content: &str) -> Vec<&str> {
    ...
    let mut offset = 0;
    while let Some(relative) = content.get(offset..).and_then(|remaining| remaining.find("cfg(")) {
        ...
        while index < bytes.len() {
            match bytes[index] {
                ...
                b')' if !quoted => {
                    depth -= 1;
                    if depth == 0 {
                        if let Some(expression) = content.get(start..index) {
...
fn extract_cfg_features(content: &str, features: &mut HashSet<String>) {
    ...
    while let Some((_, after_feature)) = remaining.split_once("feature") {
        remaining = after_feature;
        ...
        // Find closing quote
        if let Some((feature_name, _)) = after_quote.split_once('"')
            && !feature_name.is_empty()
            && is_valid_feature_name(feature_name)
        {
...
impl FeatureScanner {
    ...
    fn parse_feature_reference(s: &str) -> Option<(String, String)> {
        ...
        if let Some((dep_part, feature)) = s.split_once('/') {
