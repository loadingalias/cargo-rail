...
impl WorkspaceContext {
    ...
    fn build_inner_at(
        ...
        let snapshot = if let Some(inputs) = resolution_inputs {
            ...
            let snapshot = WorkspaceSnapshot::capture(super::snapshot::WorkspaceSnapshotCapture {
                source_root,
                source,
                cargo: Arc::clone(&cargo),
                rail_config,
                rail_config_discovery_root,
                generated_lockfile: captured_lockfile,
                resolution_inputs: inputs.clone(),
                targets: target_identities,
                derived: Arc::clone(&derived_views),
            })?;
