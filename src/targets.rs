...
pub fn get_rust_target_list() -> RailResult<Vec<String>> {
    ...
    targets.clone().ok_or_else(|| {
        RailError::message("Failed to get target list from rustc. Ensure rustc is installed and in PATH.")
    })
}
...
mod tests {
    ...
    fn test_validate_targets_valid() {
        ...
        validate_targets(&valid_targets).unwrap();
    }
