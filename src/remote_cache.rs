...
mod tests {
  ...
  fn selection_rejects_secret_like_or_noncanonical_environment_policy() {
    let url = "s3://rail-cache/team?owner=123456789012&region=us-east-1";
    assert!(RemoteCacheSelection::parse(url, None, &["CI_TOKEN".to_string()]).is_err());
    assert!(RemoteCacheSelection::parse(url, None, &["Z_FLAG".to_string(), "A_FLAG".to_string()]).is_err());
  }

  #[test]
  fn selection_approves_observed_portable_cargo_package_metadata() {
    let selection = RemoteCacheSelection::parse("s3://rail-cache/team?owner=123456789012&region=us-east-1", None, &[])
      .expect("selection");
    assert!(
      selection.approves_environment_names(&["CARGO_PKG_NAME".to_string(), "CARGO_PKG_VERSION_PATCH".to_string(),])
    );
  }
