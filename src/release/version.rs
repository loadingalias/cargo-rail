        ...
        match self {
            ...
            Self::Prerelease => {
                ...
                if current.pre.is_empty() {
                    // No prerelease - add rc.1 ("rc.1" is always valid per semver spec)
                    next.pre = semver::Prerelease::new("rc.1").unwrap_or(semver::Prerelease::EMPTY);
                } else {
                    ...
                    if let Some((prefix, num_str)) = pre_str.rsplit_once('.') {
                        if let Ok(num) = num_str.parse::<u64>() {
                            ...
                            next.pre = semver::Prerelease::new(&new_pre).unwrap_or_else(|_| current.pre.clone());
                        } else {
...
impl VersionBumper {
    ...
    pub fn update_workspace_dependency(
        manifest_path: &Path,
        dep_name: &str,
        new_version: &Version,
    ) -> RailResult<bool> {
