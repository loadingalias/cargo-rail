...
pub(crate) fn validate_label(value: &str, field: &str) -> RailResult<()> {
    ...
    if valid {
        ...
        Err(RailError::message(format!(
            "{field} must contain only letters, numbers, spaces, '-' or '_'"
        )))
    }
}
...
impl ChangelogSpec {
    ...
    pub fn diagnose(&self, commits: &[CommitRef<'_>]) -> Vec<CommitDiagnostic> {
        ...
        for commit in commits {
            ...
            let issue = match parsed.commit_type {
                ...
                Some(commit_type) if !self.known_types.iter().any(|t| &**t == commit_type) => {
                    Some(CommitIssue::UnknownType {
                        found: commit_type.to_string(),
                        suggestion: suggest_type(commit_type, self.known_types.iter().map(|t| &**t))
                            .map(str::to_string),
                    })
                }
                Some(_) => None,
    ...
    #[cfg_attr(
        not(test),
        expect(dead_code, reason = "machine-readable changelog projection for release consumers")
    )]
...
mod tests {
    ...
    fn default_render_matches_legacy_shape() {
        ...
        assert!(
            body.contains(
                "- [**breaking**] drop old API ([ccccccc](https://github.com/org/repo/commit/ccccccc3333333))"
            )
        );
