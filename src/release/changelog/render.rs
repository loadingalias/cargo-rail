...
impl EntryTemplate {
    ...
    pub fn parse(format: &str) -> Result<Self, String> {
        ...
        while let Some(open) = rest.find('{') {
            let Some((literal, placeholder)) = rest.split_at_checked(open) else {
                return Err("entry_format placeholder is not on a UTF-8 boundary".to_string());
            };
            if !literal.is_empty() {
                segments.push(Segment::Literal(literal.to_string()));
            }
            let Some(after_open) = placeholder.strip_prefix('{') else {
                return Err("entry_format placeholder is malformed".to_string());
            };
            let Some(close) = after_open.find('}') else {
                return Err(format!("unclosed placeholder in entry_format: '{placeholder}'"));
            };
            let Some((name, after_name)) = after_open.split_at_checked(close) else {
                return Err("entry_format placeholder is not on a UTF-8 boundary".to_string());
            };
            ...
            let Some(next) = after_name.strip_prefix('}') else {
                return Err("entry_format placeholder is malformed".to_string());
            };
            rest = next;
        }
...
impl UrlTemplate {
    ...
    fn write(&self, out: &mut String, value: &str) {
