...
mod tests {
    ...
    fn build_script_action_key_changes_after_recapturing_exact_bytes() {
        ...
        fs::write(source_root.path().join("crates/example/build.rs"), b"fn main(){ }")
            .expect("same-size source mutation");
    ...
    fn build_script_key_analysis_never_persists_environment_values() {
        ...
        let encoded = serde_json::to_string(
            &analyze_action_key(root.path(), controlled).expect("analyze controlled environment"),
        )
