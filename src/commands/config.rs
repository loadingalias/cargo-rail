            ...
            let host = unify
                ...
                .map(|item| {
                    item.as_str().map(str::to_string).ok_or_else(|| {
                        RailError::message("unify.transitive_host must be a string before it can be migrated")
                    })
                })
                .transpose()?
    ...
    if !legacy_msrv.is_empty() {
        let replacement = if let Some(item) = unify.get("msrv_policy") {
            ...
            let source = unify
                ...
                .map(|item| {
                    item.as_str().map(str::to_string).ok_or_else(|| {
                        RailError::message("unify.msrv_source must be a string before it can be migrated")
                    })
                })
                .transpose()?
