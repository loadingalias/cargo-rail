...
///
/// Scans the workspace for target triples mentioned in any TOML file
/// (rust-toolchain.toml, .cargo/config.toml, Cross.toml, etc.) and replaces
/// the targets array in rail.toml with the detected targets.
///
/// This ensures rail.toml always reflects what's actually configured in the
/// workspace. Use this after adding new target triples to CI config files.
pub fn run_unify_sync(workspace_root: &std::path::Path, check: bool) -> RailResult<()> {
  ...
  // Search order: rail.toml, .rail.toml, .cargo/rail.toml, .config/rail.toml
  let config_paths = [
  ...
  let exclude_refs: Vec<&std::path::Path> = config_paths.iter().map(|p| p.as_path()).collect();
  ...
  // Clean replace: detected targets become the new targets array
  if detected == existing {
    println!("targets already in sync ({} targets)", existing.len());
    return Ok(());
  }

  // Calculate diff for display
  let added: Vec<_> = detected.difference(&existing).cloned().collect();
  let removed: Vec<_> = existing.difference(&detected).cloned().collect();

  if check {
    if !added.is_empty() {
      println!("would add {} target(s):", added.len());
      for target in &added {
        println!("  + {}", target);
      }
    }
    if !removed.is_empty() {
      println!("would remove {} target(s):", removed.len());
  ...
  // Build new array with detected targets only (clean replace)
  let mut new_array = Array::new();
  for target in &detected {
    new_array.push(target.as_str());
  }

  // Update the targets array in the document
  let doc = editor.doc_mut();
  doc["targets"] = Item::Value(Value::Array(new_array));

  // Write atomically
  editor.write()?;

  println!("synced targets in {}:", config_path.display());
  if !added.is_empty() {
    for target in &added {
      println!("  + {}", target);
    }
  ...
  println!("total: {} targets", detected.len());
