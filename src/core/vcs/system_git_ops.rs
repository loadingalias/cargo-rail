...
impl SystemGit {
    ...
    pub fn read_files_bulk(&self, items: &[(String, PathBuf)]) -> RailResult<Vec<Vec<u8>>> {
        ...
        let mut stdin = child.stdin.take().ok_or_else(|| RailError::message("Failed to open stdin"))?;
...
mod tests {
    ...
    fn test_commit_touches_paths() {
        ...
        if !changed_files.is_empty() {
            ...
            let touches = git.commit_touches_paths(&head, &[changed_path.clone()]).unwrap();
