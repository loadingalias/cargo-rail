      ...
      || self
        ...
        .is_none_or(|input| {
          source_metadata.len() != input.frame.bytes
            || digest_file(&source, source_metadata.len()).ok().as_deref() != Some(&input.frame.content_digest)
        })
    {
      return Err(RailError::message(
...
pub(crate) fn record_local_placement(
  ...
  let _ = update_placement_history(receipt, observation, |entry, now| {
...
fn decode_placement_history(bytes: &[u8]) -> RailResult<PlacementHistory> {
  ...
  let history: PlacementHistory =
    serde_json::from_slice(encoded).map_err(|_| RailError::message("distributed placement history is malformed"))?;
  validate_placement_history(&history)?;
...
fn execute_mutual_tls_worker_inner(
  ...
  let capability: WorkerCapability =
    read_control_frame(&mut stream, CAPABILITY_MAGIC, CAPABILITY_TRAILER, "capability")?;
  ...
  let lease_request = LeaseRequest {
    action_id: template.action_id.clone(),
...
pub(crate) fn execute_and_admit_local_worker(
  worker: &Path,
  rustc: &OsStr,
  context: &crate::compiler::native_cache::NativeCacheContext,
  cas: &crate::cache::cas::LocalCas,
  session: &crate::compiler::native_cache::NativeCompilerSession,
  capture: &crate::compiler::native_cache::NativeActionCapture,
  base_action_key: &str,
  observation: &crate::compiler::observation::RawCompilerInvocation,
  output_paths: &crate::compiler::observation::NativeOutputPaths,
  candidate: &RustLibraryCandidate,
  ...
  let attempt = execute_local_worker(worker, rustc, candidate, staging, Some(cas), &mut timing);
  timing.record_attempt(started);
  decide_local_attempt(attempt, |result| {
    crate::compiler::native_cache::admit_distributed_rust_library_result(
      context,
      cas,
      session,
      capture,
...
fn run_worker_command(arguments: Vec<OsString>) -> RailResult<()> {
  match arguments.as_slice() {
    ...
    #[cfg(target_os = "linux")]
    [command, rustc, bubblewrap, sysroot, worker, cgroup, scratch_bytes] if command == "execute-cgroup-bubblewrap" => {
...
fn qualify_cgroup_enforcement(captured: &CapturedWorkerCapability) -> RailResult<()> {
  ...
  for probe in [CgroupProbe::Cpu, CgroupProbe::Memory, CgroupProbe::Processes] {
    ...
    let enforced = match probe {
      ...
      CgroupProbe::Processes => status.success() && outcome.process_limit_hits > 0 && outcome.memory_oom_kills == 0,
    };
    if !enforced {
      return Err(RailError::message(format!(
...
fn serve_mutual_tls_with_capability(
  ...
  while !draining.load(Ordering::Acquire) {
    ...
    if thread::Builder::new()
      ...
      .spawn(move || {
        let _active = ActiveConnection(active_connection);
        let _ = serve_mutual_tls_connection(socket, accepted, config, &captured);
      })
      .is_err()
    {
  ...
  drop(listener);
  let _ = write_worker_event(&serde_json::json!({
...
fn serve_mutual_tls_connection(
  ...
  let mut stream = rustls::StreamOwned::new(connection, socket);
  write_control_frame(&mut stream, CAPABILITY_MAGIC, CAPABILITY_TRAILER, &captured.capability)?;
  ...
  let _ = write_worker_event(&event);
  write_response(&mut stream, &envelope.request, &response.0, &response.1)?;
  stream.flush()?;
...
fn monitor_client_connection(socket: TcpStream, stopped: &AtomicBool, cancellation: mpsc::SyncSender<Cancellation>) {
  ...
  while !stopped.load(Ordering::Acquire) {
    match socket.peek(&mut byte) {
      Ok(0) => {
        let _ = cancellation.send(Cancellation::ClientLost);
        return;
      }
      Ok(_) => {
        let _ = cancellation.send(Cancellation::Requested);
        return;
      }
      Err(error)
        if matches!(
...
fn qualify_local_client(rustc: &OsStr) -> RailResult<()> {
  ...
  for slot in [
    ...
    let metadata = fs::symlink_metadata(path)?;
    if !path.starts_with(result.staging_path()) || !metadata.is_file() || crate::utils::is_symlink_or_reparse(&metadata)
    {
      return Err(RailError::message(
...
fn capture_worker_capability_for_runtime(
  ...
  let memo =
    cache.and_then(|cache| crate::compiler::collector::compiler_sysroot_memo_path_in(cache, &sysroot, &host_target));
...
fn validate_and_rebind_dep_info(
  ...
  if dependencies.is_empty()
    ...
    || !dependencies.iter().any(|dependency| {
      source_relative_path(source_virtual_path).is_some_and(|relative| dependency == &current_directory.join(relative))
    })
  {
    return Err(RailError::message(
...
fn valid_identity(value: &str, prefix: &str) -> bool {
  value.len() == prefix.len().saturating_add(64)
...
mod tests {
  ...
  #[test]
  fn request_framing_round_trips_only_canonical_bounded_input() -> RailResult<()> {
    let request = request_for(SOURCE)?;
    validate_request(&request, &capability()?)?;
    let mut encoded = Vec::new();
  ...
  #[test]
  fn input_authority_binds_modules_and_exact_dependencies_and_rejects_hostile_sets() -> RailResult<()> {
  ...
  #[test]
  fn action_identity_excludes_lease_and_workload_but_binds_operation_and_inputs() -> RailResult<()> {
    let request = request_for(SOURCE)?;
  ...
  #[test]
  fn automatic_remote_execution_requires_qualified_isolation_without_changing_compiler_equivalence() -> RailResult<()> {
  ...
  #[test]
  fn placement_class_excludes_action_names_but_binds_cost_shape_and_endpoint() -> RailResult<()> {
  ...
  #[test]
  fn automatic_placement_requires_a_conservative_fresh_material_win() -> RailResult<()> {
    let observation =
      placement_candidate(SOURCE)?.placement_observation(&fixed_identity("sha256:", 'a'), "10.0.0.1:39443")?;
    let now = 1_000_000;
  ...
  #[test]
  fn placement_history_is_canonical_bounded_and_uses_integer_ewma() -> RailResult<()> {
    let observation =
      placement_candidate(SOURCE)?.placement_observation(&fixed_identity("sha256:", 'a'), "10.0.0.1:39443")?;
    ...
    assert!(decode_placement_history(&noncanonical).is_err());
    Ok(())
  }

  #[test]
  fn connection_leases_are_fresh_and_bound_to_the_client_action_and_capability() -> RailResult<()> {
    let capability = capability()?;
    let request = request_for(SOURCE)?;
    let lease_request = LeaseRequest {
      action_id: request.action_id.clone(),
    ...
    assert!(validate_lease_grant(&first, &another_client).is_ok());
    ...
    let mut forged_capability = lease_request.clone();
  ...
  #[test]
  fn control_frames_require_canonical_bounded_json() -> RailResult<()> {
    let capability = capability()?;
    let mut encoded = Vec::new();
  ...
  #[test]
  fn client_disconnect_is_a_cancellation_signal() -> RailResult<()> {
  ...
  #[test]
  fn request_validation_rejects_forgery_and_path_shaped_arguments() -> RailResult<()> {
    let capability = capability()?;
  ...
  #[test]
  fn result_decoder_stages_only_digest_verified_authorized_slots() -> RailResult<()> {
    let request = request_for(SOURCE)?;
  ...
  #[test]
  fn success_response_slot_set_is_bound_to_the_requested_emission() -> RailResult<()> {
    let request = request_for(SOURCE)?;
    let payloads = tempfile::tempdir()?;
    let frames = success_frames(payloads.path())?;
    let timing = worker_timing(&request, &frames);
  ...
  #[test]
  fn result_decoder_rejects_corruption_and_mismatched_authority() -> RailResult<()> {
    let request = request_for(SOURCE)?;
    let encoded = response_bytes(&request)?;
    let header_length = u32::from_le_bytes(
      encoded[8..12]
  ...
  #[test]
  fn response_rejects_worker_timing_that_does_not_match_the_attempt() -> RailResult<()> {
    let request = request_for(SOURCE)?;
    let payloads = tempfile::tempdir()?;
    let frames = success_frames(payloads.path())?;
    let timing = worker_timing(&request, &frames);
  ...
  #[test]
  fn rejection_response_carries_no_artifact_payload() -> RailResult<()> {
    let request = request_for(SOURCE)?;
  ...
  #[test]
  fn compiler_failure_preserves_termination_and_only_diagnostic_streams() -> RailResult<()> {
    let request = request_for(SOURCE)?;
    let frames = vec![
  ...
  #[test]
  fn local_attempt_falls_back_only_before_the_visible_effect_boundary() -> RailResult<()> {
    use std::cell::Cell;
  ...
  #[cfg(unix)]
  #[test]
  fn local_client_collapses_a_malformed_worker_to_cold_without_admitting_bytes() -> RailResult<()> {
    use std::os::unix::fs::PermissionsExt as _;
    ...
    let rustc = String::from_utf8(selected.stdout).map_err(|_| RailError::message("test rustc path was not UTF-8"))?;
