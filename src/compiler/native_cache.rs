            ...
            witness
                ...
                .map(|relative| {
                    let index = generated
                        ...
                        .binary_search_by(|entry| entry.path.as_str().cmp(relative))
                        .map_err(|_| {
                            RailError::message("native result witness is absent from current generated state")
                        })?;
                    let NativeSourceEntryKind::RegularFile {
                        content_digest, mode, ..
                    ...
                    Ok(FileObservation {
                        path: ObservationPath::capture(
                            &generated.root_spelling.join(relative),
                            source_root,
                            source_root,
                        ),
                        content_digest: content_digest.clone(),
    ...
    if apple_linked_observation(observation) {
        match (&witness.linker, generations) {
            (Some(LinkerWitness::Apple(linker)), Some(generations)) => {
                validate_apple_linker_witness(linker).is_ok()
                    && validate_apple_linker_generations(generations, linker).is_ok()
            }
        ...
        match (&witness.linker, generations) {
            (Some(LinkerWitness::Elf(linker)), Some(generations)) => {
                validate_elf_linker_witness(linker).is_ok()
                    && validate_elf_linker_generations(generations, linker).is_ok()
            }
...
fn capture_apple_linker_witness(
    ...
    let bytes = read_bounded(certificate, MAX_APPLE_LINK_CERTIFICATE_LEN)?;
...
fn capture_elf_linker_witness(
    ...
    let capture_elf_file = |path: &Path, budget: &mut NativeCaptureBudget| {
        capture_link_file(path, started, budget).map_err(|error| {
...
fn read_apple_link_driver_evidence(path: &Path) -> RailResult<AppleLinkDriverEvidence> {
    let bytes = read_bounded(path, MAX_APPLE_LINK_CERTIFICATE_LEN)?;
...
fn is_rustc_temporary_name(name: &str) -> bool {
    name.strip_prefix("rustc")
        .is_some_and(|suffix| suffix.len() == 6 && suffix.bytes().all(|byte| byte.is_ascii_alphanumeric()))
}
...
fn revalidate_apple_linker_witness(
    ...
    let trusted_generations = generations.filter(|generations| {
        installation_authority.map(installation_authority_identity).as_ref()
...
fn revalidate_captured_namespace(captured: &NativeNamespaceCapture, role: &str) -> RailResult<()> {
    ...
    for expected in &captured.guard.entries {
        ...
        let metadata = fs::symlink_metadata(&path)?;
        if crate::utils::is_symlink_or_reparse(&metadata)
            || native_metadata_guard(&path, &metadata)? != expected.metadata
        {
            return Err(RailError::message(format!(
        ...
        let children = collect_native_directory_children(
            fs::read_dir(&absolute_directory)?.filter_map(|entry| match entry {
                Ok(entry) => {
                    ...
                    (!excluded_roots.iter().any(|excluded| absolute.starts_with(excluded)))
                        .then_some(Ok(entry.file_name()))
                }
    ...
    pub(crate) fn is_direct_invocation() -> bool {
        std::env::args_os().next().is_some_and(|invoked| {
            ...
            file_name == Some(OsStr::new(DIRECT_WRAPPER_NAME))
                || (file_name == Some(OsStr::new(DIRECT_WORKER_NAME))
                    && std::env::var_os(DIRECT_LAUNCHER_ENV).is_some())
        })
    }
...
impl NativeCompilerValidation {
    ...
    pub(crate) fn remote_environment_is_approved(&self, approved_names: &[String]) -> bool {
        self.compiler_environment_names
            .iter()
    ...
    fn revalidate_publication(
        ...
        self.validate_object().map_err(|error| {
            NativePublicationRevalidationFailure::new("cold_protocol_changed_before_admission", error)
        })?;
        session.validate_object().map_err(|error| {
            NativePublicationRevalidationFailure::new("cold_protocol_changed_before_admission", error)
        })?;
        proof.validate_object().map_err(|error| {
            NativePublicationRevalidationFailure::new("cold_protocol_changed_before_admission", error)
        })?;
        let current_source_root_identity = path_identity(source_root).map_err(|error| {
            NativePublicationRevalidationFailure::new("cold_session_changed_before_admission", error)
        })?;
        if session.source_root_identity != current_source_root_identity
            ...
            || !self.compiler_environment_names.iter().eq(proof
                .approved_environment
                .entries
                .iter()
        ...
        .map_err(|error| {
            NativePublicationRevalidationFailure::new("cold_linker_inputs_changed_before_admission", error)
        })?;
        if selected_action != self.action_key {
            return Err(NativePublicationRevalidationFailure::new(
        ...
        while index < arguments.len() {
            ...
            match argument {
                ...
                _ if matches!(argument.as_bytes().first(), Some(b'-'))
                    ...
                    && argument.len() > 2 =>
                {
                    let option = argument.get(..2).ok_or("distributed_argument_shape_ineligible")?;
                    let value = argument.get(2..).ok_or("distributed_argument_shape_ineligible")?;
                    parsed.capture_lint(option, Some(value))?;
                }
...
fn cache_key_compiler_arguments(
    ...
    while index < arguments.len() {
        ...
        match argument.as_str() {
            ...
            "-L" => {
                let value =
                    next.ok_or_else(|| RailError::message("native compiler library search value is missing"))?;
                identity.push(argument.clone());
    ...
    for output in observed {
        ...
        let Some((index, _)) = outputs.iter().enumerate().find(|(index, expected)| {
            !matched.contains(index)
                ...
                && output.executable
                    == matches!(expected.role.as_str(), "executable" | "proc_macro" | "dylib" | "cdylib")
...
pub(crate) fn fast_bypass_reason(program: &OsStr, arguments: &[OsString]) -> Option<&'static str> {
    ...
    for (index, argument) in arguments.iter().enumerate() {
        ...
        if matches!(argument, "-h" | "--help" | "-V" | "--version" | "-vV" | "--print")
        ...
        if let Some(value) = inline_or_next(argument, next(), "--extern") {
            ...
            if let Some(reason) =
                dependency_artifact_bypass_reason(Path::new(artifact).extension().and_then(OsStr::to_str))
            {
                return Some(reason);
            }
        }
...
fn compiler_argument_bypass_reason<T: AsRef<str>>(
    ...
    while index < arguments.len() {
        ...
        let consumes_next = match argument {
            ...
            "--remap-path-prefix"
                if next.is_some_and(|value| distributed_workspace_remap_at(value, current_directory)) =>
            {
                true
            }
...
pub(crate) fn metadata_from_environment() -> Option<CompilerCacheWrapperMetadata> {
    ...
    u64::try_from(encoded.len())
        .is_ok_and(|length| length <= MAX_SESSION_BYTES)
...
///
/// This value is immediately matched once. Only the cold store payload is boxed,
/// keeping hit, bypass, and operational-failure decisions compact.
...
pub(crate) struct OuterCacheStore {
    pub(crate) recorder: InvocationRecorder,
    pub(crate) capture: NativeActionCapture,
...
pub(crate) fn configure_outer(program: &OsStr, arguments: &[OsString], command: &mut Command) -> OuterCacheAction {
        ...
        && let Some(selection) = active_remote_selection()
    {
        if !selection.direct_transport_supported() {
            ...
            let lookup = remote.and_then(|store| {
                let store = store
                    .ok_or_else(|| crate::remote_cache::RemoteStoreError::unavailable("remote store is inactive"))?;
                let started = Instant::now();
        ...
        crate::cache::cas::NativeActionLookup::Packed(cached) => {
            ...
            match attempt_local_packed_reuse(
                ...
                PackedLocalReuse::Reject(reason) => {
                    ...
                    drop(cached);
                    drop(cas.quarantine_packed_native_action(&action_key, reason));
        ...
        && selection.approves_environment_names(&environment_names)
    {
        ...
        metrics.remote_timing.store_connect.record(started);
        match remote {
            Ok(Some(remote)) => match attempt_direct_remote_reuse(DirectRemoteReuseRequest {
                cas: &cas,
                remote,
                selection,
                session: &session,
        ...
        let attempt = if let Some((identity, policy)) = distributed_remote.as_ref() {
            ...
            match decision {
                crate::compiler::distributed::PlacementDecision::Delegate => {
                    ...
                    if let (Some(receipt), Some(placement)) =
                        (context.installation.as_ref(), distributed_placement.as_ref())
                    {
                        match &attempt {
                            crate::compiler::distributed::LocalAttemptDecision::Completed(_) => {
                                crate::compiler::distributed::record_remote_placement(
                                    receipt,
                                    placement,
                                    started.elapsed(),
                                    true,
            ...
            crate::compiler::distributed::execute_and_admit_local_worker(worker, rustc, admission)
        } else {
...
fn attempt_local_packed_reuse(
    ...
    let action_matches = validation.action_key() == authority.action_key();
    let result_matches = validation.result_key() == authority.result_key();
    let session_matches = validation.session_identity == session.identity;
    let class_matches = validation.class == session.class;
    if !action_matches
        || !result_matches
        || !session_matches
        || !class_matches
...
struct DirectRemoteReuseRequest<'a> {
    cas: &'a LocalCas,
    remote: &'a crate::remote_cache::RemoteStore,
    selection: &'a crate::remote_cache::RemoteCacheSelection,
    ...
    observation: &'a RawCompilerInvocation,
    output_paths: &'a NativeOutputPaths,
    metrics: &'a mut NativeCacheMetrics,
}

fn attempt_direct_remote_reuse(request: DirectRemoteReuseRequest<'_>) -> DirectRemoteReuse {
    let DirectRemoteReuseRequest {
        cas,
        remote,
        selection,
        session,
    ...
    if cas
        .commit_packed_native_action_revalidated(packed, &validation, |validation| {
            ...
            recapture_bytes = capture.revalidate_before_restore_commit(
                observation,
                &context.source_root,
...
pub(crate) fn admit_distributed_rust_library_result(
    authority: crate::compiler::distributed::DistributedAdmissionAuthority<'_>,
    result: crate::compiler::distributed::StagedExecutionResult,
    ...
    let crate::compiler::distributed::DistributedAdmissionAuthority {
        context,
        cas,
        session,
    ...
    let result_authority = NativeResultAuthority {
        ...
        source_root_spelling: &context.source_root_spelling,
    };
    let (prepared, proof) = match prepare_distributed_result(result_authority, result) {
        Ok(prepared) => prepared,
...
#[derive(Clone, Copy)]
struct NativeRestoreAuthority<'a> {
    initial_capture: &'a NativeActionCapture,
    current_observation: &'a RawCompilerInvocation,
    output_paths: &'a NativeOutputPaths,
    source_root: &'a Path,
...
fn prepare_registered_restore(
    ...
    authority: NativeRestoreAuthority<'_>,
    metrics: &mut NativeCacheMetrics,
) -> RailResult<PreparedNativeRestore> {
    let NativeRestoreAuthority {
        initial_capture,
        current_observation,
        output_paths,
        source_root,
    ...
    raw.observed_reads =
        current_observed_reads(initial_capture, &validation.witness, current_observation, source_root)?;
...
fn rename_restore_write_through(
    ...
    for attempt in 0..MAX_ATTEMPTS {
        ...
        match crate::windows_fs::rename_write_through(source, destination, destination_identity.is_some()) {
            Ok(()) => return Ok(()),
            Err(error)
                if matches!(error.raw_os_error(), Some(5 | 32 | 33)) && attempt.saturating_add(1) < MAX_ATTEMPTS =>
            {
...
impl NativeRestoreTransaction {
    ...
    fn rollback(&mut self) -> RailResult<()> {
        ...
        if let NativeRestoreTransactionState::Committed(commit) = &self.state {
            validate_authorized_restore_transaction(
                &self.paths,
    ...
    match member {
        NativeRestoreMember::Output {
            ...
            if &current == source_identity
                && restore_file_matches_digest(&destination, source_identity, content_digest)?
            {
                Ok(Some(destination))
            } else {
    ...
    T: Serialize + for<'de> Deserialize<'de>,
{
    ...
    let value: T = serde_json::from_slice(&bytes)
        .map_err(|_| RailError::message(format!("native restore {label} is malformed")))?;
        ...
        for (scope, path) in output_binding_paths(output, source_root)? {
            for (form, path) in output_path_forms(&path) {
                for (representation, rendered) in encoded_output_path_forms(&path, encoding) {
                    let portable =
                        format!("/cargo-rail/native-output/v3/{role}/{scope}/{form}/{representation}").into_bytes();
                    replacements.push(if to_portable {
                        (rendered, portable)
                    } else {
                        (portable, rendered)
    ...
    !path.is_empty()
        && bytes.windows(path.len()).enumerate().any(|(index, window)| {
            ...
            before.is_none_or(|byte| byte.is_ascii_whitespace() || matches!(byte, b':' | b'=' | b'\'' | b'"'))
                && after
                    .is_none_or(|byte| byte.is_ascii_whitespace() || matches!(byte, b'/' | b'\\' | b':' | b'\'' | b'"'))
        })
}
...
pub(crate) fn configure_apple_link_adapter(command: &mut Command, arguments: &[OsString]) -> bool {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        for argument in arguments {
            ...
            let rustc_temporary = parent
                .file_name()
                .and_then(OsStr::to_str)
                .and_then(|name| name.strip_prefix("rustc"))
                .is_some_and(|suffix| suffix.len() == 6 && suffix.bytes().all(|byte| byte.is_ascii_alphanumeric()));
...
pub(crate) fn finalize_apple_link_adapter() -> bool {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let Ok(bytes) = read_bounded(&certificate, MAX_APPLE_LINK_CERTIFICATE_LEN) else {
            return false;
        };
...
fn publish_direct_remote_result(
    ...
    let cached = match cas.native_action(validation.action_key()) {
        ...
        Ok(crate::cache::cas::NativeActionLookup::Packed(_) | crate::cache::cas::NativeActionLookup::Miss(_))
        | Err(_) => {
            ...
            staging.ok_or("local_cache_staging_failed").and_then(|staging| {
                prepare_cold_result(
                    NativeResultAuthority {
                        ...
                        expected_base_action: &base_action_key,
    ...
    let publication = prepared.and_then(|(prepared, proof)| {
        ...
        (|| {
            ...
            if let Some(candidate) = link_candidate.as_deref() {
                ...
                cas.publish_native_link_candidate(candidate, validation.action_key())
                    .map_err(|_| admission_failure)?;
            }
...
#[derive(Clone, Copy)]
struct NativeResultAuthority<'a> {
    session: &'a NativeCompilerSession,
    initial_capture: &'a NativeActionCapture,
    expected_base_action: &'a str,
    current_observation: &'a RawCompilerInvocation,
    output_paths: &'a NativeOutputPaths,
    source_root: &'a Path,
    source_root_spelling: &'a Path,
...
fn prepare_distributed_result(
    ...
    let prepared: RailResult<_> = (|| {
        ...
        let dep_info =
            portable_dep_info_output_bindings(&localized_dep_info, output_paths, source_root, initial_capture)?;
        ...
        if base_action_key(&session.identity, &session.class, &cache_observation, initial_capture)?
            != expected_base_action
        {
            return Err(RailError::message(
...
fn validate_staged_output(
    ...
    let metadata = fs::symlink_metadata(path)?;
    let staged = FileObservation::capture(path, path.parent().unwrap_or_else(|| Path::new("/")), Path::new("/"))?;
...
fn write_benchmark_coverage_event(
    ...
    write_benchmark_coverage_invocation(BenchmarkCoverageInvocation {
        directory,
        status,
        reason,
        action_key,
...
struct BenchmarkCoverageInvocation<'a> {
    directory: &'a Path,
    status: CompilerCacheWrapperStatus,
    reason: &'a str,
    action_key: Option<&'a str>,
...
fn write_benchmark_coverage_invocation(invocation: BenchmarkCoverageInvocation<'_>) -> RailResult<()> {
    let BenchmarkCoverageInvocation {
        directory,
        status,
        reason,
        action_key,
...
pub(crate) mod tests {
    ...
    fn write_apple_link_driver_evidence(
        ...
        fs::write(path, serde_json::to_vec(&evidence).expect("driver-input encoding"))
            .expect("Apple linker driver inputs");
    }
    ...
    fn apple_link_witness_binds_found_missing_toolchain_and_endogenous_inputs() {
        ...
        assert!(
            revalidate_apple_linker_witness(&witness, Some(&generations), Some(&authority))
                .expect("same-content fallback")
                > 0
        );
        assert!(
            revalidate_apple_linker_witness(&witness, Some(&generations), None).expect("foreign installation fallback")
                > 0
        );

        fs::write(&found, b"stable-two").expect("mutated stable input");
        revalidate_apple_linker_witness(&witness, Some(&generations), Some(&authority)).unwrap_err();
    }
    ...
    fn apple_link_witness_recognizes_dynamic_rustc_object_names() {
        ...
        for (role, crate_type, linked_name, object_name) in [
            ...
            let (witness, _, _) =
                capture_apple_linker_witness(&observation, &outputs, &certificate, &driver_inputs, None)
    ...
    fn apple_link_witness_rejects_uncertified_or_user_selected_objects() {
        ...
        capture_apple_linker_witness(&observation, &outputs, &certificate, &driver_inputs, None).unwrap_err();
    ...
    fn distributed_candidate_requires_closed_source_dependency_and_argument_authority() {
        let result: RailResult<()> = (|| {
            ...
            let command =
                normalization.normalized_local_command(OsStr::new("rustc"), workspace.path(), temporary.path())?;
    ...
    #[test]
    fn distributed_workspace_remap_accepts_only_the_exact_current_directory() {
        let result: RailResult<()> = (|| {
            let workspace = tempfile::tempdir()?;
    ...
    #[cfg(unix)]
    #[test]
    fn native_staging_parent_modes_are_independent_of_the_callers_umask() {
        let result: RailResult<()> = (|| {
    ...
    #[test]
    fn distributed_result_requires_live_revalidation_before_l1_admission() {
        let result: RailResult<()> = (|| {
            let workspace = tempfile::tempdir()?;
            fs::write(
                workspace.path().join("Cargo.toml"),
            ...
            let result_authority = NativeResultAuthority {
                session: &session,
                initial_capture: &capture,
                expected_base_action: &base_action,
                current_observation: &observation,
                output_paths: &output_paths,
            ...
            let (drift_prepared, drift_proof) =
                prepare_distributed_result(result_authority, drift_result).map_err(RailError::message)?;
            ...
            cas.store_native_revalidated(drift_prepared, |validation| {
                validation
            ...
            assert!(prepare_distributed_result(result_authority, changed,).is_err());
            Ok(())
        })();
        result.unwrap();
    }
    ...
    fn empty_environment_selectors_are_authoritative() {
        for (revision, first, second) in [
            ...
            cas.native_environment_selector(&key)
    ...
    fn revalidated_store_runs_selector_publication_before_action_commit() {
        ...
        cas.store_native_revalidated(prepared_cas_fixture(validation), |_| {
            assert!(matches!(
    ...
    fn restore_rejects_absent_mismatched_and_conflicted_selectors() {
        let validate = |first: Option<Vec<String>>, second: Option<Vec<String>>| -> Result<(), RestorePublishFailure> {
            ...
            if let Some(second) = second {
                assert_eq!(
                    cas.publish_native_environment_selector(&base_action, &second)
                        .expect("second selector publication"),
                    crate::cache::cas::NativeEnvironmentSelectorPublication::Diverged
                );
            }
            let crate::cache::cas::NativeActionLookup::Hit(hit) =
    ...
    fn action_binds_complete_state_while_result_binds_selected_witness() {
        ...
        assert_eq!(
            action,
            action_key(&session.identity, &session.class, &observed_changed, &capture)
                .expect("selection-neutral action"),
    ...
    fn complete_action_capture_hashes_bytes_instead_of_trusting_size() {
        ...
        let initial_action =
            action_key(&session.identity, &session.class, &observation, &initial).expect("initial action");
    ...
    fn mutate_until_native_guard_changes(observed: &Path, initial: &NativeMetadataGuard, mut mutate: impl FnMut()) {
        let started = Instant::now();
        loop {
            mutate();
            let metadata = fs::symlink_metadata(observed).expect("mutated native metadata");
            let current = native_metadata_guard(observed, &metadata).expect("mutated native guard");
            if &current != initial {
                return;
            }
            assert!(
                started.elapsed() < Duration::from_secs(1),
                "the filesystem generation must advance after a native input mutation"
            );
    ...
    fn generated_restore_revalidation_rejects_a_same_size_x_to_y_to_x_mutation() {
        ...
        let initial_guard = native_metadata_guard(
            &generated,
            &fs::symlink_metadata(&generated).expect("generated metadata"),
        )
        .expect("generated guard");
        mutate_until_native_guard_changes(&generated, &initial_guard, || {
            fs::write(&generated, b"pub const VALUE: u8 = 2;\n").expect("same-size mutation");
    ...
    fn native_search_restore_revalidation_rejects_a_same_size_x_to_y_to_x_mutation() {
        ...
        let initial_guard = native_metadata_guard(&archive, &fs::symlink_metadata(&archive).expect("archive metadata"))
            .expect("archive guard");
        mutate_until_native_guard_changes(&archive, &initial_guard, || {
    ...
    fn restore_revalidation_rejects_a_same_size_x_to_y_to_x_mutation() {
        ...
        let initial_guard = native_metadata_guard(&source, &fs::symlink_metadata(&source).expect("source metadata"))
            .expect("source guard");
        mutate_until_native_guard_changes(&source, &initial_guard, || {
            fs::write(&source, b"pub const VALUE: u8 = 2;\n").expect("same-size mutation");
    ...
    fn restore_revalidation_rejects_a_transient_namespace_entry() {
        ...
        let namespace = source.parent().expect("source namespace");
        let initial_guard = native_metadata_guard(
            namespace,
            &fs::symlink_metadata(namespace).expect("source namespace metadata"),
        )
        .expect("source namespace guard");
        mutate_until_native_guard_changes(namespace, &initial_guard, || {
    ...
    fn elf_linker_witness_accepts_a_rustc_object_removed_after_linking() {
        ...
        fs::write(&driver_inputs, serde_json::to_vec(&evidence).expect("driver evidence"))
            .expect("driver evidence file");
        let outputs = NativeOutputPaths {
    ...
    fn publication_root_must_remain_inside_the_standard_target_root() {
        ...
        validated_output_parent(&valid, source.path()).unwrap();
        ...
        validated_output_parent(&source_mutation, source.path()).unwrap_err();

        let escaped = metadata_output_paths(
            external.path().join("fixture.d"),
            external.path().join("libfixture.rmeta"),
        );
        validated_output_parent(&escaped, source.path()).unwrap_err();
    }
    ...
    fn root_bearing_environment_is_portable_only_while_unobserved() {
        ...
        capture.witness(&environment_only, Path::new("/workspace")).unwrap_err();
    ...
    fn metadata_and_rlib_outputs_share_one_verified_contract() {
        ...
        let outputs = [
            ...
            (
                "rlib",
                RLIB_SLOT,
                "libfixture-0123456789abcdef.rlib",
                b"rlib".as_slice(),
            ),
        ]
        .into_iter()
        .map(|(role, slot, file_name, bytes)| NativeCompilerOutput {
            role: role.to_string(),
            slot: slot.to_string(),
    ...
    fn dep_info_materialization_rebinds_only_verified_output_paths() {
        ...
        let translated =
            translate_dep_info_output_bindings(&portable, &validation, &outputs, source_root.path(), &capture)
                .expect("portable dep-info");
        assert_eq!(
            translated,
        ...
        let generated =
            b"build-one/debug/deps/libfixture-0123456789abcdef.rmeta: build-one/debug/deps/out/generated.rs\n";
    ...
    fn dep_info_source_root_rebinding_preserves_exact_path_spelling() {
        ...
        let converged_bindings = dep_info_source_root_replacements(&canonical_package, PORTABLE_PACKAGE_ROOT, false)
            .expect("converged bindings");
        ...
        for (cold_root, portable_root) in first_bindings {
            ...
            let portable =
                portable_dep_info_source_roots(&cold_root, workspace.path(), &package_capture(&first_package))
