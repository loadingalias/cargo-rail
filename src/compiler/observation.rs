...
pub(crate) fn attach_build_script_result_dependencies(
    ...
    for binding in bindings {
        ...
        if !dependents.contains_key(binding.package.as_str()) {
            for manifest in manifests.iter_mut() {
                if manifest.unit.package != binding.package
...
fn begin_compiler_invocation(
    ...
    let dependency_artifacts = parsed
        ...
        .filter_map(|(name, path)| {
            capture_one_file(&path, &current_dir, source_root, &mut bypasses, "dependency_artifact")
                .map(|file| (name, file))
        })
        .collect();
...
mod tests {
    ...
    fn native_link_path_inference_matches_platform_rustc_names() {
        ...
        for (crate_type, crate_name, expected) in [
            ...
            let parsed =
                ParsedArguments::parse(&arguments, Path::new("/workspace"), CompilerMode::Rustc, &mut bypasses);
    ...
    fn cargo_fresh_is_result_metadata_not_artifact_identity() {
        ...
        let output =
            FileObservation::capture(&output_path, directory.path(), directory.path()).expect("output identity");
    ...
    .expect("dep info");
        ...
        let (reads, environment) = parse_dep_info(&directory.path().join("unit.d"), directory.path(), directory.path())
            .expect("parse dep info");
    ...
    fn raw_loader_ignores_typed_sidecars_and_binds_content_addressed_names() {
        ...
        load_raw(directory.path()).unwrap_err();
    }
}
