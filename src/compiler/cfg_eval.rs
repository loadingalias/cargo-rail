...
mod tests {
    ...
    fn cargo_target_constraint_uses_cargos_complete_cfg_grammar() {
        ...
        cargo_target_constraint_matches("cfg(all(unix)", "target", &cfg).unwrap_err();
    ...
    fn test_feature_selections_cover_compound_positive_and_negative_conditions() {
        assert_eq!(
            feature_selections_for_cfg(
                r#"all(feature = "a", feature = "b", not(feature = "c"), target_arch = "x86_64")"#
            ),
