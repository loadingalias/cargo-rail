      ...
      || toolchain.host_target() != self.rustc_host
    {
      return Err(RailError::with_help(
        format!(
          ...
          self.rustc_release, self.rustc_commit, self.rustc_host, selected.release, selected.commit, selected.host
        ),
...
impl CompilerFactDoctestSysroot {
  ...
  fn stage(
    ...
    let runtime_library_generation = Some(
      crate::utils::stable_file_generation(&runtime_library)
  ...
  pub(crate) fn revalidate(&self) -> RailResult<()> {
    ...
    #[cfg(windows)]
    {
      ...
      if crate::windows_fs::observe_file(&self._rustc_guard)? != crate::windows_fs::observe_file(&File::open(rustc)?)?
...
impl Drop for CompilerFactDoctestSysroot {
  fn drop(&mut self) {
    #[cfg(unix)]
    {
      ...
      let _ = fs::set_permissions(self.root.join("bin"), fs::Permissions::from_mode(0o700));
      let _ = fs::set_permissions(self.root.join("lib"), fs::Permissions::from_mode(0o700));
...
fn stage_component(component: &CompilerFactDriverComponent) -> RailResult<CompilerFactDriverExecutionCapability> {
  ...
  let directory_text = directory_path
    ...
    .ok_or_else(|| RailError::message("staged compiler fact driver path cannot be expressed in a sandbox profile"))?;
...
mod tests {
  ...
  fn build_authority_is_all_or_nothing_and_target_bound() {
    ...
    assert!(CompilerFactDriverAuthority::from_fields(incomplete).is_err());
  ...
  fn verbose_compiler_identity_requires_exact_unique_fields() {
    ...
    assert!(RustcVerboseIdentity::parse("rustc 1.95.0").is_err());
    assert!(RustcVerboseIdentity::parse(&format!("{verbose}\nhost: duplicate")).is_err());
  }

  #[test]
  fn typed_driver_rejects_every_untrusted_outer_or_inner_wrapper() {
    assert!(validate_exclusive_wrapper_authority(None, None).is_ok());
    ...
    assert!(
      validate_exclusive_wrapper_authority(Some(OsStr::new("outer-wrapper")), Some(OsStr::new("workspace-wrapper")),)
        .is_err()
    );
  }
  ...
  fn sibling_component_is_bound_to_exact_bytes_and_provenance() {
    ...
    let component =
      CompilerFactDriverComponent::discover_with_authority(&authority, &executable, directory.path().to_path_buf())
