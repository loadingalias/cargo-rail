...
use crate::compiler::model::{
  ...
  MAX_COMPILER_DIAG_CACHE_ENTRIES, TargetEvidence,
};
...
pub(crate) struct CompilerEvidenceValidation {
  version: u32,
  action_key: String,
...
impl CompilerEvidenceValidation {
  ...
  pub(crate) fn collector_version(&self) -> u32 {
    self.collector_version
  }

  pub(crate) fn observations(&self) -> &[crate::compiler::observation::CompilationObservationManifest] {
    &self.observations
  }

  pub(crate) fn matches(&self, key: &CompilerDiagKey) -> bool {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct CompilerEvidenceObject {
  version: u32,
...
impl CompilerEvidenceObject {
  ...
  pub(crate) fn identity(&self) -> RailResult<String> {
    if self.version != EVIDENCE_OBJECT_VERSION {
      return Err(RailError::message(
  ...
  pub fn get(&mut self, key: &CompilerDiagKey) -> Option<&CompilerDiagEntry> {
    ...
    for candidate in candidates {
      ...
      self.record_prior(
        ...
        candidate.validation.collector_version(),
      );
      if hit.is_none()
        && candidate.validation.collector_version() == COLLECTOR_VERSION
        && candidate.validation.matches(key)
      {
        hit = Some(CompilerDiagEntry {
          key: key.clone(),
          evidence: candidate.evidence.evidence().clone(),
          generated_at_unix_ms,
          collector_version: candidate.validation.collector_version(),
          observations: candidate.validation.observations().to_vec(),
        });
      }
    }
...
fn prune_entries(
  ...
  let mut candidates = entries
    .into_iter()
    .filter(|(id, entry)| {
      id == &entry.key.stable_id() && now.saturating_sub(entry.generated_at_unix_ms) <= limits.age_ms
    })
    .collect::<Vec<_>>();
