...
mod tests {
  ...
  fn set_manifest_without_its_object_is_a_cache_miss() {
    ...
    let validation = CompilerFactEvidenceValidation::set(key.clone(), vec![reference.clone()]).expect("set validation");
    let evidence = CompilerEvidenceObject::from_compiler_fact_set(vec![reference]).expect("set evidence");
    cas
      .store_compiler_evidence(CompilerEvidenceStoreRequest {
        validation: &validation,
        evidence: &evidence,
      })
  ...
  fn corrupt_fact_object_cannot_authorize_a_hit() {
    ...
    let evidence = fs::read_dir(results)
      ...
      .filter_map(Result::ok)
      .find(|entry| {
        fs::read(entry.path()).is_ok_and(|bytes| bytes.windows(15).any(|window| window == b"\"kind\":\"object\""))
      })
