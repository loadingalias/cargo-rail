...
/// Portable compiler path identity: logical crate name, then typed definition path.
///
/// This deliberately excludes rustc's `StableCrateId`, whose `-Cmetadata`
/// input is Cargo-invocation-specific. Equal identities from ambiguous
/// same-named crates are resolved conservatively to every matching physical
/// declaration by the Surface graph.
