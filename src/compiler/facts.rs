...
pub(crate) fn load_discovered_doctest_fragment(
  ...
  let unit_identity = untrusted.object.unit.identity.clone();
...
fn validate_span(span: CompilerFactSpan, sources: &[CompilerFactSource]) -> RailResult<()> {
  ...
  if span.start >= span.end || span.end > source.bytes {
    return Err(RailError::message(
...
mod tests {
  ...
  fn fragment_rejects_noncanonical_or_duplicate_tables() {
    ...
    let mut duplicate_source = fragment.clone();
  ...
  fn fragment_rejects_unbounded_or_unknown_json() {
    ...
    assert!(ValidatedCompilerFactObject::from_bytes(&oversized, &expectation(&fragment()).object).is_err());
    ...
    object.push(b'\n');
    assert!(ValidatedCompilerFactObject::from_bytes(&object, &expectation(&fragment).object).is_err());
  }
  ...
  fn manufactured_driver_fragment_passes_the_stable_admission_boundary() {
    ...
    let message = String::from_utf8(cargo.stdout)
      ...
      .find(|event| {
        event["reason"] == "compiler-message" && event["message"]["code"]["code"] == COMPILER_FACT_ANNOUNCEMENT_CODE
      })
