...
impl CompilerFactSession {
    ...
    pub(crate) fn write_with_typed(
        ...
        let (_file, path) = capability.keep().map_err(|error| {
            RailError::message(format!("failed to retain compiler fact capability: {}", error.error))
        })?;
        Ok(path)
    }
...
impl CompilerFactTypedSession {
    ...
    pub(crate) fn authorize_invocation(
        ...
        let invocation_identity = if doctest_builder {
            raw.compiler_fact_invocation_identity_with_input(Some(doctest_input_identity.ok_or_else(|| {
                RailError::message("typed doctest compiler invocation has no captured stdin identity")
            })?))?
        } else {
