...
fn parse_index(output: &[u8], exclusions: &SourceExclusions) -> RailResult<Vec<IndexEntry>> {
    ...
    for record in output.split(|byte| *byte == 0).filter(|record| !record.is_empty()) {
        ...
        let raw_path = std::str::from_utf8(&record[tab + 1..])
            .map_err(|_| RailError::message("git index path is not valid UTF-8"))?;
        ...
        match record.first().copied() {
            ...
            Some(b'2') => {
                ...
                let original = records.next().ok_or_else(|| {
                    RailError::message("invalid Git porcelain-v2 rename record: missing original path")
                })?;
                let original = std::str::from_utf8(original)
                    .map_err(|_| RailError::message("Git rename source path is not valid UTF-8"))?;
            ...
            Some(b'?') if record.get(1) == Some(&b' ') => {
                let path = text
                    .strip_prefix("? ")
                    .ok_or_else(|| RailError::message("invalid Git porcelain-v2 untracked record"))?;
...
fn read_diff(
    ...
    for change in &mut changes {
        let related_path = match &change.relation {
            Some(
                ChangeRelation::RenamedFrom(path) | ChangeRelation::RenamedTo(path) | ChangeRelation::CopiedFrom(path),
            ) => Some(path),
            None => None,
        };
...
fn parse_name_status(output: &[u8]) -> RailResult<Vec<ParsedChange>> {
    ...
    while let Some(status) = fields.next() {
        let status =
            std::str::from_utf8(status).map_err(|_| RailError::message("Git diff status is not valid UTF-8"))?;
        let code = status
            .as_bytes()
            .first()
            .copied()
        ...
        match code {
            b'A' | b'M' | b'D' | b'T' => {
                ...
                let kind = match code {
                    ...
                    _ => {
                        return Err(RailError::message(format!(
                            "Git diff emitted unsupported status '{status}'"
                        )));
                    }
                };
...
mod tests {
    ...
    fn repository_paths_and_symlink_targets_reject_non_utf8() {
        ...
        RepositoryPath::new(invalid).unwrap_err();
        SourceTreeEntry::symlink(Path::new("link"), invalid).unwrap_err();
    }
    ...
    fn name_status_parser_retains_rename_copy_and_type_semantics() {
        let parsed = parse_name_status(
            b"M\0modified.rs\0T\0typed.rs\0R100\0old.rs\0new.rs\0C100\0source.rs\0copy.rs\0D\0gone.rs\0",
        )
        .unwrap();
