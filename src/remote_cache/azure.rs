...
impl Policy for RequestMetricsPolicy {
    async fn send(&self, context: &Context, request: &mut Request, next: &[Arc<dyn Policy>]) -> PolicyResult {
        ...
        let next = next.split_first().ok_or_else(|| {
            azure_core::Error::with_message(ErrorKind::Other, "Azure pipeline has no transport policy")
        })?;
    ...
    async fn get_token(
        ...
        if selected != CREDENTIAL_UNSELECTED {
            return self
                ...
                .get(selected)
                .ok_or_else(|| {
                    azure_core::Error::with_message(ErrorKind::Credential, "Azure credential selection is invalid")
                })?
...
impl AzureBackend {
    ...
    pub(super) fn get_marker(&self, key: &str) -> RemoteStoreResult<Option<Vec<u8>>> {
        let Some(download) = self.download(key, super::object::PROTOCOL_MARKER.len() as u64, RequestKind::Marker)?
        else {
            return Ok(None);
        };
            ...
            Ok(Err(error)) => {
                return match classify_error(&error, kind) {
                    ...
                    RequestFailure::Precondition => {
                        Err(RemoteStoreError::integrity("remote read returned a precondition"))
                    }
