...
impl ObjectStore {
  ...
  fn entry_key(&self, identity: &str) -> RemoteStoreResult<String> {
    ...
    Ok(self.object_suffix(&format!("entries/{}/{identity}", &digest[..2])))
  }
...
fn encode_unique_entry(
  ...
  {
    let mut encoder = zstd::stream::write::Encoder::new(&mut compressed, COMPRESSION_LEVEL).map_err(io_unavailable)?;
...
fn encode_entry(record: &EntryRecord, mut payload: Option<&mut File>) -> RemoteStoreResult<(File, u64)> {
  ...
  let header_length =
    u32::try_from(header.len()).map_err(|_| RemoteStoreError::integrity("remote entry header length is invalid"))?;
...
fn encode_canonical<T: Serialize>(value: &T, label: &str) -> RemoteStoreResult<Vec<u8>> {
  let bytes =
    serde_json::to_vec(value).map_err(|_| RemoteStoreError::integrity(format!("remote {label} encoding failed")))?;
...
pub(super) fn decode_entry_prelude(prelude: &[u8; ENTRY_PRELUDE_BYTES as usize]) -> RemoteStoreResult<usize> {
  ...
  if header_length == 0 || header_length as u64 > MAX_METADATA_BYTES {
    return Err(RemoteStoreError::integrity(
