...
pub(super) fn stop_all(receipt: &InstallationReceipt) {
  ...
  for identity in identities {
      ...
      && state.validate(&identity, &state.authority).is_ok()
    {
      ...
      let _ = client.stop();
      let _ = crate::cache::installation::remove_coordinator_state_if(receipt, &identity, &bytes);
    }
...
fn run_server(identity: &str) -> RemoteStoreResult<()> {
  ...
  let shared = Arc::new(ServerShared {
    ...
    publish_token: state.publish_token.clone(),
    store: OnceLock::new(),
...
fn write_environment_names(stream: &mut TcpStream, names: &[String]) -> RemoteStoreResult<()> {
  ...
  let count =
    u16::try_from(names.len()).map_err(|_| RemoteStoreError::integrity("remote environment count is out of range"))?;
